vl_gr_violin_passeios_geral <- function(dados_stats) {
  ...
  media <- 
    dados_graph_base %>%
    group_by(Turista) %>%
    summarise(media = round(mean(n_passeios, na.rm = TRUE), 2))
  ...
  shapiro_com <- shapiro.test(dados_graph_base$n_passeios[dados_graph_base$Turista == "Com \n motivo"])
  shapiro_sem <- shapiro.test(dados_graph_base$n_passeios[dados_graph_base$Turista == "Sem \n motivo"])
  
  # Escolha do teste de comparação com base na normalidade
  if (shapiro_com$p.value > 0.05 & shapiro_sem$p.value > 0.05) {
    teste <- t.test(n_passeios ~ Turista, data = dados_graph_base)
    tipo_teste <- "T-Test"
  } else {
    teste <- wilcox.test(n_passeios ~ Turista, data = dados_graph_base, exact = FALSE)  # Mann-Whitney U
  ...
  label_shap_com <- 
    list(
      ...
      y = max(dados_graph_base$n_passeios, na.rm = TRUE) + 5
    )
  ...
  label_dif_test <- list(
    ...
    y = min(dados_graph_base$n_passeios - 0.15, na.rm = TRUE)
  )

  g <- 
    dados_graph_base %>%
    ggplot(aes(y = n_passeios, x = Turista, fill = Turista)) +
    scale_y_log10(limits = c(min(dados_graph_base$n_passeios)-0.2, max(dados_graph_base$n_passeios) + 5))+
    ...
    # Finalização
    theme_classic() +
    labs(title = "Número de destinos alcançados de barco por turistas", y = "Número de Destinos") +
