vl_gr_violin_perm <- function(dados_stats) {
  ...
  media <-
    dados_graph_base %>%
    group_by(Turista) %>%
    summarise(media = mean(Permanencia, na.rm = TRUE) %>% round(2))

  # Testes de normalidade
  shapiro_com <- shapiro.test(dados_graph_base$Permanencia[dados_graph_base$Turista == "Com \n motivo"])
  shapiro_sem <- shapiro.test(dados_graph_base$Permanencia[dados_graph_base$Turista == "Sem \n motivo"])
  
  # Escolha do teste de comparação com base na normalidade
  if (shapiro_com$p.value > 0.05 & shapiro_sem$p.value > 0.05) {
    teste <- t.test(Permanencia ~ Turista, data = dados_graph_base)
    tipo_teste <- "T-Test"
  } else {
    teste <- wilcox.test(Permanencia ~ Turista, data = dados_graph_base, exact = FALSE)  # Mann-Whitney U
  ...
  label_shap_com <- 
    list(
      ...
      y = max(dados_graph_base$Permanencia, na.rm = TRUE) * 1.4
    )
  ...
  label_dif_test <- list(
    ...
    y = min(dados_graph_base$Permanencia, na.rm = TRUE)
  )
  
  # Criar gráfico com as anotações
  g <- 
    dados_graph_base %>%
    ggplot(aes(y = Permanencia, x = Turista, fill = Turista)) +
    # Importante mexer no eixo já pela ordem de cosntrução do graph
    scale_y_log10(limits = c(min(dados_graph_base$Permanencia)-15, max(dados_graph_base$Permanencia) * 1.7)) +
    # geoms
    ...
    # Finalização
    theme_classic() +
    labs(title = "Dias de permanência do turista em Cananéia", y = "Número de dias") +
